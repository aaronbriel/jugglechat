...
"""
NOTE: On 11/15 This code was pulled directly from:
https://github.com/deepset-ai/FARM/blob/master/examples/question_answering_crossvalidation.py

All that was changed is data_dir and filename as well as routing logger to a file. This was run to replicate similar 
metric results as deepset achieved 
in their paper: "COVID-QA: A Question Answering Dataset for COVID-19", but with my custom dataset based on their 
faq_covidbert.csv set combined with the set from "Collecting Verified COVID-19 Question Answer Pair", refined.
TODO improve text here

Also had to change batch_size=25 to resolve "RuntimeError: CUDA out of memory", as per 
https://github.com/huggingface/transformers/issues/906

As I specified in https://github.com/deepset-ai/COVID-QA/issues/112:
I was able to come quite close to replicating the results of their paper using an Ubuntu 18.04 VM running a Tesla 
V100 GPU with 128G disk space:
XVAL EM: 0.26151560178306094
XVAL f1: 0.5858967501101285

However, when running question_answering_crossvalidation.py with my dataset the results are not as good as 
what is observed with their dataset or even with the baseline referenced in the paper:
XVAL EM: 0.21554054054054053
XVAL f1: 0.4432141443807887

It is difficult to equate this with ultimate performance as out of the box question answering ability with respect
to Covid-19 is superior with my model.
"""
...
def question_answering_crossvalidation():
    ...
    data_dir = Path("./")
    filename = "squad-jhu_covid_qa-refined-cleaned.json"
    xval_folds = 5
    dev_split = 0
    evaluate_every = 0
